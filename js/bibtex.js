...
// http://www.cc.gatech.edu/~dellaert/publications/html/Dellaert.html
...
function BibTexEntry(text) {
  ...
  var i = text.indexOf('{');
  if (i == -1) throw ("BibTexEntry: no { delimiter - "+text);
  var j = text.lastIndexOf('}');
  if (j == -1) throw ("BibTexEntry: no } delimiter - "+text);
  ...
  for (var k in pairs) {
    ...
    if (pair.length>2) { // to get rid of whitespace fields, fix !
      ...
      if (keyvalue.length!=2) throw ("BibTexEntry: invalid key-value pair: [" + pair + "]" + " in " + text);
...
BibTexEntry.prototype.toList = function()
{
  ...
  case "InCollection":
    ...
    break;
  case "InBook":
      string += "<li>booktitile : " + this.title;
      break;
  case "TechReport":
...
BibTexEntry.prototype.render = function(database)
{
  ...
  function lookup(entry,database,field) {
	...
	if(this.pages) {
	    string += ":" + span("pages",this.pages.replace("--","-"));  
	}
...
function load(url) {
  try {
    ...
    bibtex.error("There seemed to have been an error loading the bibtex file ("+url+"):<br>" + err);
  }
}
